...
// Release-note parsing adapted from WhisperKiller.
...
enum UpdateReleaseNotes {
    nonisolated private static func parseChangelog(_ markdown: String) -> [ChangelogEntry] {
        ...
        for (index, section) in sections.enumerated() {
            ...
            if index == 0 && !section.hasPrefix("[") && !section.hasPrefix("v") && section.first?.isNumber != true {
                continue
            }
